...
Under the hood, the `props` argument passed to any Solid component is a proxy. When accessing a property on `props` with dot notation, we are implicitly calling the getter for that proxy. This does not work the same if we instead destructured the property from `props`.
