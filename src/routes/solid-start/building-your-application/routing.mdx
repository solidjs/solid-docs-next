...
SolidStart traverses your `routes` directory, collects all of the routes, and then makes them accessible using the [`<FileRoutes />`](/solid-start/reference/router-reference/file-routes).
...
type LoadFnReturnType = { user: { name: string } };
...
} satisfies RouteDefinition<string, LoadFnReturnType>;

const UserLayout: Component<{ data: LoadFnReturnType }> = (props) => {
  return (
    <div>
      <h1>User</h1>
